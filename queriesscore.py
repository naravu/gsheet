...
creds_dict = st.secrets["gcp_service_account"]   # Cloud mode
...
# --- New Button: Save to Google Sheets ---
if st.button("Save to Google Sheets"):
    if not name.strip():
        ...
        for q in questions:
            selected_opts = responses.get(q["question"], [])
            for opt in q["options"]:
                sheet.append_row([
                    name,
                    q["question"],
                    opt["text"],
                    opt["score"] if opt["text"] in selected_opts else 0
                ])
